...
const createHealthProfessionalRegistration = async (req, res) => {
  ...
  let professionalCredentialsPath = null;
  if (req.file) {
    professionalCredentialsPath = req.file.path;
  }
  ...
  try {
    // Validate required fields - include district (password is now optional as it's generated by admin)
    ...
    try {
      ...
      const counselorResult = await client.query(
        ...
        [userId, areaOfSpecification, licenseRegistrationNumber, alternativeNumber || null, parseInt(yearOfExperience), currentInstitutions, professionalCredentialsPath || null, district, 'pending']
      );
