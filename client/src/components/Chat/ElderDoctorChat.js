...
import styles from './ElderDoctorChat.module.css';

const ElderDoctorChat = ({ currentUser, selectedDoctor, onClose }) => {
  ...
  const fetchMessages = async (isBackgroundRefresh = false) => {
    if (!currentUser?.user_id || !selectedDoctor?.user_id) {
      console.log('Missing user IDs in ElderDoctorChat:', {
        currentUser: currentUser?.user_id,
        selectedDoctor: selectedDoctor?.user_id,
    ...
    try {
      ...
      console.log('ElderDoctorChat - Fetching messages between:', {
      ...
      if (response.success) {
        ...
        if (hasUnreadMessages) {
          await messagesApi.markAsRead(selectedDoctor.user_id, currentUser.user_id);
        }
      }
    } catch (error) {
  ...
  const sendMessage = async (e) => {
    ...
    try {
      ...
      console.log('ElderDoctorChat - Sending message:', {
        from: currentUser.user_id,
        to: selectedDoctor.user_id,
        senderType: 'elder',
        receiverType: 'doctor',
  ...
  }, [currentUser?.user_id, selectedDoctor?.user_id]); // Only re-run if user/doctor changes
      ...
      <div className={styles.chatHeader}>
        <div className={styles.doctorInfo}>
          ...
          <span className={styles.doctorDetails}>
            {selectedDoctor.specialization} • {selectedDoctor.years_experience} years experience
          </span>
        </div>
        <button className={styles.closeButton} onClick={onClose}>×</button>
      </div>
...
export default ElderDoctorChat;
