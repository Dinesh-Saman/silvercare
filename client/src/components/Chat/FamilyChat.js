...
import styles from './FamilyChat.module.css';

const FamilyChat = ({ currentUser, selectedFamilyMember, onClose }) => {
  ...
  const fetchMessages = async (isBackgroundRefresh = false) => {
    if (!currentUser?.user_id || !selectedFamilyMember?.user_id) {
      console.log('Missing user IDs in FamilyChat:', {
    ...
    try {
      ...
      console.log('FamilyChat - Fetching messages between:', {
  ...
  const sendMessage = async (e) => {
    ...
    try {
      ...
      console.log('FamilyChat - Sending message:', {
      ...
      <div className={styles.chatHeader}>
        <div className={styles.familyMemberInfo}>
          ...
          <span className={styles.familyMemberDetails}>
            {selectedFamilyMember.family_member_email} • {selectedFamilyMember.elders_count} elder(s)
          </span>
        </div>
        <button className={styles.closeButton} onClick={onClose}>×</button>
      </div>
