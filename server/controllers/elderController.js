...
const createAppointment = async (req, res) => {
  ...
  try {
    ...
    // If it's an online appointment, generate meeting link immediately
    let meetingLink = null;
    if (appointmentType === 'online') {
      try {
        console.log('Generating meeting link for online appointment...');
        
        // Get elder and doctor names for the meeting
        const elderName = elderResult.rows[0].name;
        ...
        console.log('Meeting link generated successfully:', meetingLink);
      } catch (err) {
        console.error('Error generating meeting link:', err);
        // Don't fail the appointment creation if meeting link generation fails
      }
    }
    ...
    try {
      ...
      if (tempBooking.appointment_type === 'online') {
        try {
          ...
          // Get elder and doctor names for the meeting
          const elderQuery = await client.query('SELECT name FROM elder WHERE elder_id = $1', [tempBooking.elder_id]);
          ...
          const elderName = elderQuery.rows[0]?.name || 'Patient';
          const doctorName = doctorQuery.rows[0]?.doctor_name || 'Doctor';
          ...
          // Update appointment with meeting link
          await client.query(
            'UPDATE appointment SET meeting_link = $1 WHERE appointment_id = $2',
            [meetingDetails.meetingUrl, newAppointment.appointment_id]
          );
      ...
      res.status(201).json({
        ...
        appointment: { ...newAppointment, meeting_link: meetingLink },
        payment: payment
      });
